...
export interface PluggingResult {
    filesUpdated: number;
...
export async function plugFrontend(
    targetPath: string,
    backendUrl: string,
    scanResult?: ScanResult
): Promise<PluggingResult> {
    const result: PluggingResult = {
        filesUpdated: 0,
        replacementsCount: 0,
        injectionsCount: 0,
    };
    ...
    async function plugFile(filePath: string, isRecursive = false) {
        ...
        if (globalUtilityFiles.includes(fileName)) {
            ...
            content = content.replace(apiCallRegex, (match, prefix, pathVal, suffix) => {
                if (pathVal.startsWith("http")) return match;
                updated = true;
            ...
            for (const endpoint of scanResult.endpoints) {
                if (endpoint.purpose === "login" || endpoint.purpose === "signup") {
                    ...
                    content = content.replace(funcRegex, (match, head, body, tail) => {
                        ...
                        if (trimmedBody.length > 500) return match;
...
`;
    ...
    async function walk(dir: string) {
        ...
        for (const file of files) {
            ...
            if (stat.isDirectory()) {
                if (!excludeDirs.includes(file)) await walk(fullPath);
            } else await plugFile(fullPath);
        }
    }

    try {
        const stat = await fs.stat(targetPath);
        if (stat.isFile()) await plugFile(targetPath);
