 ...
 */
export async function scanFrontend(targetPath: string): Promise<ScanResult> {
    ...
    function extractFieldsFromHTML(content: string): { fields: string[], purpose: "login" | "signup" | "generic", formId?: string }[] {
        const results: { fields: string[], purpose: "login" | "signup" | "generic", formId?: string }[] = [];
        ...
        while ((formMatch = formRegex.exec(content)) !== null) {
            ...
            // Determine purpose
            let purpose: "login" | "signup" | "generic" = "generic";
    ...
    function updateEndpoint(pathValue: string, method: string, fields: string[], purpose?: EndpointInfo["purpose"], formId?: string) {
